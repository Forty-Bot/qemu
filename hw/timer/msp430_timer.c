...
static void msp430_timer_recalculate(MSP430TimerState *t)
{
    ...
    if (event_time_ns) {
        trace_msp430_timer_schedule(t->last_updated + event_time_ns,
                                    event_time_ns, t->last_updated);
...
static bool msp430_timer_update(MSP430TimerState *t)
{
    ...
    trace_msp430_timer_event(now, now - t->last_updated, t->last_updated,
                             actual_cycles, t->event_cycles);
...
static void msp430_timer_reset_hold(Object *obj, ResetType type)
{
    ...
    t->last_updated = 0;
    t->event_cycles = 0;
    memset(t->ccia, 0, sizeof(t->ccia));
    memset(t->ccib, 0, sizeof(t->ccib));
    memset(t->capture_unread, 0, sizeof(t->capture_unread));
    memset(t->out, 0, sizeof(t->out));
    t->down = 0;

    t->ctl = 0;
    t->r = 0;
    memset(t->cctl, 0, sizeof(t->cctl));
    memset(t->ccr, 0, sizeof(t->ccr));
    memset(t->cl, 0, sizeof(t->cl));
}
...
static void msp430_timer_class_init(ObjectClass *oc, void *data)
{
    ...
    rc->phases.hold = msp430_timer_reset_hold;
}
