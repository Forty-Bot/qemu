...
static void basic_timer_reset_hold(Object *obj, ResetType type)
{
    ...
    timer_del(&bt->timer);
    qemu_irq_lower(bt->irq);

    bt->last_updated = 0;
    bt->event_cycles = 0;
    bt->ctl = g_random_int();
    bt->cnt[0] = g_random_int();
    bt->cnt[1] = g_random_int();
    basic_timer_recalculate(bt);
}
...
static void basic_timer_class_init(ObjectClass *oc, void *data)
{
    ...
    rc->phases.hold = basic_timer_reset_hold;
}
