...
static void usart_reset(MSP430USARTState *usart)
{
    usart->ctl = 0x01;
    usart->tctl = 0x01;
    usart->rctl = 0x00;
...
static void usart_reset_hold(Object *obj, ResetType type)
{
    MSP430USARTState *usart = MSP430_USART(obj);

    if (type != RESET_TYPE_GUEST) {
        usart->mctl = g_random_int();
        usart->br = g_random_int();
        usart->rxbuf = g_random_int();
        usart->txbuf = g_random_int();
    }

    usart_reset(usart);
}

static void usart_puc(void *opaque, int irq, int level)
{
    if (level) {
        usart_reset(opaque);
    }
...
static void usart_init(Object *obj)
{
    ...
    qdev_init_gpio_in_named(DEVICE(d), usart_puc, "puc", 1);
