...
typedef struct {
    ...
    void (*reset)(MSP430USCIState *usci);
    bool is_a;
} MSP430USCIClass;
...
static int usci_uart_can_receive(void *opaque)
{
    ...
    if (qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) < usci->rx_next) {
        return 0;
    }
...
static void usci_write(void *opaque, hwaddr uaddr, uint64_t val, unsigned size)
{
    ...
    case A_CTL1: {
        ...
        bool ssel_changed = (val ^ usci->ctl1) & R_CTL1_SSEL_MASK;
...
static void usci_reset(MSP430USCIState *usci)
{
    usci->ctl0 = 0x00;
    usci->ctl1 = 0x01;
    usci->br = 0x0000;
...
static void usci_reset_hold(Object *obj, ResetType type)
{
...
static void usci_puc(void *opaque, int irq, int level)
{
    MSP430USCIState *usci = opaque;
    const MSP430USCIClass *uc = MSP430_USCI_GET_CLASS(usci);
...
static void usci_init(Object *obj)
{
    ...
    qdev_init_gpio_in_named(DEVICE(d), usci_puc, "puc", 1);
...
static void usci_b_reset(MSP430USCIState *usci)
{
    MSP430USCIBState *usci_b = MSP430_USCI_B(usci);
