...
static void lcd_set_out_m(MSP430LCDState *lcd, int i)
{
    ...
    for (j = 0; j < 8; j++) {
        ...
        if (!lcd->clk_on || !(lcd->ctl & R_CTL_ON_MASK)) {
            level = -1;
        } else if (!(lcd->ctl & R_CTL_SON_MASK)) {
            level = 0;
...
static void lcd_reset(MSP430LCDState *lcd)
{
    lcd->ctl = 0x00;
    lcd_set_out(lcd);
}

static void lcd_reset_hold(Object *obj, ResetType type)
{
    ...
    if (type != RESET_TYPE_GUEST) {
        ...
        for (i = 0; i < sizeof(lcd->m); i++) {
            lcd->m[i] = g_random_int();
        }
    }
    lcd_reset(lcd);
}

static void lcd_puc(void *opaque, int irq, int level)
{
    if (level) {
        lcd_reset(opaque);
    }
...
static void lcd_init(Object *obj)
{
    ...
    qdev_init_gpio_in_named(DEVICE(lcd), lcd_puc, "puc", 1);
