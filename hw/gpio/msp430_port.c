...
static void port_write(void *opaque, hwaddr addr, uint64_t val, unsigned size)
{
    ...
    case A_IN:
        qemu_log_mask(LOG_GUEST_ERROR,
                      "msp430_port: write to read-only register IN.\n");
        return;
    case A_OUT:
...
static void port_irq_write(void *opaque, hwaddr addr, uint64_t val, unsigned size)
{
    ...
    case A_IFG:
        ...
        return;
    case A_IES:
        port->ifg |= (val ^ port->ies) & (val ^ port->in);
        port->ies = val;
        port_recalculate_irq(port, old_irq);
        return;
    case A_IE:
...
static void port_reset(MSP430PortState *port)
{
    int i;
...
static void port_reset_hold(Object *obj, ResetType type)
{
    MSP430PortState *port = MSP430_PORT(obj);

    if (type != RESET_TYPE_GUEST) {
        port->in = g_random_int();
        port->out = g_random_int();
        port->ies = g_random_int();
    }
    port_reset(port);
}

static void port_puc(void *opaque, int irq, int level)
{
    if (level) {
        port_reset(opaque);
    }
...
static void port_init(Object *obj)
{
    ...
    qdev_init_gpio_in_named(DEVICE(port), port_puc, "puc", 1);
...
static void port16_puc(void *opaque, int irq, int level)
{
    MSP430Port16State *port16 = opaque;
    int i;

    for (i = 0; i < ARRAY_SIZE(port16->puc); i++) {
        qemu_set_irq(port16->puc[i], level);
    }
}
...
static void port16_init(Object *obj)
{
    ...
    qdev_init_gpio_in_named(DEVICE(port16), port16_puc, "puc", 1);
    for (i = 0; i < ARRAY_SIZE(port16->puc); i++) {
        port16->puc[i] = qdev_get_gpio_in_named(DEVICE(&port16->port[i]), "puc", 0);
    }

    qdev_pass_gpios(DEVICE(&port16->port[0]), DEVICE(obj), NULL);
    qdev_pass_gpios(DEVICE(&port16->port[1]), DEVICE(obj), NULL);
}
