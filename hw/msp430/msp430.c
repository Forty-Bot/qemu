...
void msp430_load_kernel(MSP430State *s, const char *filename)
{
    ...
    if (load_elf_ram_sym(filename, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                         false, EM_MSP430, 0, 0, cs->as, true,
                         msp430_cio_symbol_callback, cs) >= 0) {
        return;
    }
...
static void msp430_puc_edge(void *opaque, int irq, int level)
{
    MSP430State *s = opaque;

    qemu_irq_raise(s->puc_level);
}

static void msp430_puc_ack(void *opaque, int irq, int level)
{
    MSP430State *s = opaque;

    qemu_irq_lower(s->puc_level);
}
...
struct timer_config {
    hwaddr io, iv;
    const char *type;
    int timers, irq, aclk_cci, caout_cci;
};

static const struct timer_config timer_configs[] = {
    { 0x160, 0x12e, .aclk_cci = -1 },
    { 0x180, 0x11e, .aclk_cci = -1 },
};
...
typedef struct {
    ...
    bool wdt_plus;
    bool sfr_has_reset_cause;
    bool has_tlv, has_aclk_cci;
...
#define TLV_LEN     64
#define TLV_DCO     0x01
#define TLV_EMPTY   0xfe

static void gen_tlv(uint8_t *tlv, MSP430BCMState *bcm)
{
    uint8_t sum_lo = 0, sum_hi = 0;
    int i;
    ...
    tlv[54] = TLV_DCO;
    tlv[55] = 8;
    bcm_find_closest(bcm, 16000000, &tlv[56], &tlv[57]);
    bcm_find_closest(bcm, 12000000, &tlv[58], &tlv[59]);
    bcm_find_closest(bcm, 8000000, &tlv[60], &tlv[61]);
    bcm_find_closest(bcm, 1000000, &tlv[62], &tlv[63]);

    for (i = 2; i < TLV_LEN; i += 2) {
        sum_lo ^= tlv[i];
        sum_hi ^= tlv[i + 1];
    }
                                 ...
                                 Error **errp)
{
    ...
    object_property_set_bool(OBJECT(usart), "has_i2c", config->i2c,
                             &error_fatal);
...
static void msp430_realize(DeviceState *dev, Error **errp)
{
    ...
    DeviceState *sfr = DEVICE(&s->sfr);
    DeviceState *puc = DEVICE(&s->puc);
    DeviceState *puc_latch = DEVICE(&s->puc_latch);
    DeviceState *nmi_ack = DEVICE(&s->nmi_ack);
    DeviceState *reset_ack = DEVICE(&s->reset_ack);
    DeviceState *aclk_gpio = DEVICE(&s->aclk_gpio);
    DeviceState *aclk_cci = DEVICE(&s->aclk_cci);
    DeviceState *clock, *wdt;
    int i, next_puc = 0, next_puc_latch = 0;

    /* PUC sources */
    object_property_set_int(OBJECT(puc_latch), "num-lines", 1, &error_fatal);
    qdev_realize(puc_latch, NULL, &error_fatal);

    /* SRAM */
    memory_region_init_ram(&s->sram, OBJECT(dev), "sram", mc->sram_size,
                           &error_fatal);
    if (mc->sram_size > 0xa00) {
        memory_region_add_subregion(get_system_memory(), 0x1100, &s->sram);
        memory_region_init_alias(&s->mirror, OBJECT(dev), "mirror", &s->sram,
                                 0, 0x800);
        memory_region_add_subregion(get_system_memory(), 0x200, &s->mirror);
    } else {
    ...
    object_property_set_int(OBJECT(puc), "num-lines", mc->puc_count,
                            &error_fatal);
    qdev_connect_gpio_out(puc_latch, 0, qdev_get_gpio_in(puc, 0));
    qdev_realize(puc, NULL, &error_fatal);

    qdev_init_gpio_in_named(dev, msp430_puc_ack, "puc_ack", 1);
    qdev_init_gpio_in_named(dev, msp430_puc_edge, "puc_edge", 1);
    qdev_init_gpio_out_named(dev, &s->puc_level, "puc_level", 1);
    qdev_connect_gpio_out(puc, next_puc++,
                          qdev_get_gpio_in_named(dev, "puc_edge", 0));

    object_property_set_int(OBJECT(reset_ack), "num-lines", 3, &error_fatal);
    ...
    qdev_connect_gpio_out(reset_ack, 1, qdev_get_gpio_in_named(sfr, "ack",
                                                               SFR_RST));
    qdev_connect_gpio_out(reset_ack, 2,
                          qdev_get_gpio_in_named(dev, "puc_ack", 0));
    ...
    for (i = 0; i < NUM_IRQS; i++) {
        DeviceState *irq;
        int n = ctpop32(mc->sfr_map[i]) + (i == IRQ_RESET);
        int j, k = n;
        ...
        qdev_connect_gpio_out(irq, 0, qdev_get_gpio_in(cpu, i));
        qdev_realize(irq, NULL, &error_fatal);

        if (i == IRQ_RESET) {
            qdev_connect_gpio_out_named(dev, "puc_level", 0,
                                        qdev_get_gpio_in(irq, --k));
        }
    ...
    object_property_set_bool(OBJECT(sfr), "has_reset_cause",
                             mc->sfr_has_reset_cause, &error_fatal);
    sysbus_mmio_map(SYS_BUS_DEVICE(sfr), 0, 0);
    qdev_connect_gpio_out(puc, next_puc++,
                          qdev_get_gpio_in_named(sfr, "puc", 0));
    ...
    case CLOCK_BCMP:
        ...
        object_property_set_bool(OBJECT(clock), "has_xts", mc->bcmp_has_xts,
                                 &error_fatal);
        sysbus_mmio_map(SYS_BUS_DEVICE(clock), 1, 0x53);
    ...
    case CLOCK_BCM:
        ...
        sysbus_mmio_map(SYS_BUS_DEVICE(clock), 0, 0x56);
    ...
    case CLOCK_FLLP:
        ...
        qdev_connect_gpio_out_named(cpu, "cpuoff", 0,
                                    qdev_get_gpio_in_named(clock, "cpuoff", 0));
        qdev_connect_gpio_out_named(cpu, "oscoff", 0,
                                    qdev_get_gpio_in_named(clock, "oscoff", 0));
        break;
    default:
    ...
    qdev_connect_gpio_out(puc, next_puc++,
                          qdev_get_gpio_in_named(clock, "puc", 0));
    ...
    if (clock_has_source(s->xt2) || clock_is_enabled(s->xt2)) {
    ...
    if (mc->has_aclk_cci) {
        aclk_gpio = DEVICE(&s->aclk_gpio);
        qdev_connect_clock_in(aclk_gpio, "clk", s->aclk);
        qdev_realize(aclk_gpio, NULL, &error_fatal);

        aclk_cci = DEVICE(&s->aclk_cci);
        object_property_set_int(OBJECT(aclk_cci), "num-lines",
                                ARRAY_SIZE(mc->timer), &error_fatal);
        qdev_connect_gpio_out(aclk_gpio, 0, qdev_get_gpio_in(aclk_cci, 0));
        qdev_realize(aclk_cci, NULL, &error_fatal);
    }

    if (mc->mpy_type) {
        SysBusDevice *mpy = SYS_BUS_DEVICE(&s->mpy);

        sysbus_mmio_map(mpy, 0, 0x130);
        qdev_connect_gpio_out(puc, next_puc++,
                              qdev_get_gpio_in_named(DEVICE(mpy), "puc", 0));
        sysbus_realize(mpy, &error_fatal);
    }

    if (mc->wdt_plus) {
        wdt = DEVICE(&s->wdtp);
        qdev_connect_clock_in(wdt, "mclk", s->mclk);
        /* TODO: clock requests */
    } else {
        wdt = DEVICE(&s->wdt);
    }
    sysbus_mmio_map(SYS_BUS_DEVICE(wdt), 0, 0x120);
    qdev_connect_clock_in(wdt, "aclk", s->aclk);
    qdev_connect_clock_in(wdt, "smclk", s->smclk);
    qdev_connect_gpio_out(puc, next_puc++,
                          qdev_get_gpio_in_named(wdt, "puc_in", 0));
    qdev_connect_gpio_out_named(cpu, "ack", IRQ_WDT,
                                qdev_get_gpio_in_named(wdt, "ack", 0));
    qdev_connect_gpio_out_named(wdt, "nmi", 0, qdev_get_gpio_in(sfr, SFR_NMI));
    qdev_connect_gpio_out_named(wdt, "irq", 0, qdev_get_gpio_in(sfr, SFR_WDT));
    qdev_connect_gpio_out_named(wdt, "puc_out", 0,
                                qdev_get_gpio_in(puc_latch, next_puc_latch++));
    sysbus_realize(SYS_BUS_DEVICE(wdt), &error_fatal);

    for (i = 0; i < ARRAY_SIZE(mc->port); i++) {
        if (mc->port[i].present) {
            qdev_connect_gpio_out(puc, next_puc++,
                                  qdev_get_gpio_in_named(DEVICE(&s->port[i]),
                                                         "puc", 0));
    ...
    for (i = 0; i < ARRAY_SIZE(mc->port16); i++) {
        if (mc->port16[i].present) {
            qdev_connect_gpio_out(puc, next_puc++,
                                  qdev_get_gpio_in_named(DEVICE(&s->port16[i]),
                                                         "puc", 0));
            msp430_realize_port16(s, SYS_BUS_DEVICE(&s->port16[i]),
                                  &mc->port16[i], &error_fatal);
        }
    }
    ...
    for (i = 0; i < ARRAY_SIZE(mc->timer); i++) {
        if (mc->timer[i].type) {
            ...
            msp430_realize_timer(s, SYS_BUS_DEVICE(timer), &mc->timer[i],
                                 &error_fatal);
            if (mc->timer[i].aclk_cci >= 0) {
                qemu_irq cci = qdev_get_gpio_in_named(timer, "ccib",
                                                      mc->timer[i].aclk_cci);

                qdev_connect_gpio_out(aclk_cci, i, cci);
            }
        }
    }
    ...
    for (i = 0; i < ARRAY_SIZE(mc->usci_a); i++) {
        if (mc->usci_a[i].present) {
            DeviceState *usci = DEVICE(&s->usci_a[i]);

            qdev_prop_set_chr(usci, "chardev", serial_hd(i));
            qdev_connect_gpio_out(puc, next_puc++,
                                  qdev_get_gpio_in_named(usci, "puc", 0));
            msp430_realize_usci(s, SYS_BUS_DEVICE(usci), &mc->usci_a[i],
                                &error_fatal);
        }
    }
    ...
    for (i = 0; i < ARRAY_SIZE(mc->usci_b); i++) {
        if (mc->usci_b[i].present) {
            DeviceState *usci = DEVICE(&s->usci_b[i]);

            qdev_connect_gpio_out(puc, next_puc++,
                                  qdev_get_gpio_in_named(usci, "puc", 0));
            msp430_realize_usci(s, SYS_BUS_DEVICE(usci), &mc->usci_b[i],
                                &error_fatal);
        }
    }
    ...
    for (i = 0; i < ARRAY_SIZE(mc->usart); i++) {
        if (mc->usart[i].present) {
            DeviceState *usart = DEVICE(&s->usart[i]);

            qdev_prop_set_chr(usart, "chardev", serial_hd(i));
            qdev_connect_gpio_out(puc, next_puc++,
                                  qdev_get_gpio_in_named(usart, "puc", 0));
            msp430_realize_usart(s, SYS_BUS_DEVICE(usart), &mc->usart[i],
                                 &error_fatal);
        }
    }

    if (mc->has_lcd) {
        DeviceState *lcd = DEVICE(&s->lcd);

        sysbus_mmio_map(SYS_BUS_DEVICE(&s->lcd), 0, 0x90);
        qdev_connect_gpio_out(puc, next_puc++,
                              qdev_get_gpio_in_named(lcd, "puc", 0));
        qdev_connect_clock_in(lcd, "clk", s->bt.lcdclk);
        sysbus_realize(SYS_BUS_DEVICE(lcd), &error_fatal);
    }
}
...
static void msp430_init(Object *obj)
{
    ...
    object_initialize_child(obj, "puc", &s->puc, TYPE_SPLIT_IRQ);
    object_initialize_child(obj, "puc-latch", &s->puc_latch, TYPE_OR_IRQ);
    for (i = 0; i < NUM_IRQS; i++) {
    ...
    switch (mc->clock_type) {
    case CLOCK_BCM:
        object_initialize_child(obj, "bcm", &s->bcm, TYPE_MSP430_BCM);
        break;
    case CLOCK_BCMP:
        object_initialize_child(obj, "bcm+", &s->bcm, TYPE_MSP430_BCMP);
        break;
    case CLOCK_FLLP:
    ...
    if (mc->has_aclk_cci) {
        object_initialize_child(obj, "aclk-gpio", &s->aclk_gpio, TYPE_CLK_GPIO);
        object_initialize_child(obj, "aclk-cci", &s->aclk_cci, TYPE_SPLIT_IRQ);
    }
    ...
    if (mc->wdt_plus) {
        object_initialize_child(obj, "wdt+", &s->wdtp, TYPE_MSP430_WDTP);
    } else {
        object_initialize_child(obj, "wdt", &s->wdt, TYPE_MSP430_WDT);
    }
...
static void msp430_class_render(MSP430Class *mc, const char *name)
{
    ...
    mc->puc_count = 4; /* CPU, SFR, clock, WDT */
    mc->puc_count += !!mc->mpy_type;

    for (i = 0; i < ARRAY_SIZE(mc->port); i++) {
        mc->puc_count += mc->port[i].present;
    }

    for (i = 0; i < ARRAY_SIZE(mc->port16); i++) {
        mc->puc_count += mc->port16[i].present;
    }
    ...
    mc->puc_count += mc->has_lcd;

    for (i = 0; i < ARRAY_SIZE(mc->timer); i++) {
        if (mc->timer[i].type && mc->timer[i].aclk_cci >= 0) {
            mc->has_aclk_cci = true;
        }
    }
...
static void msp430x1xx_class_init(ObjectClass *oc, void *data)
{
    MSP430Class *mc = MSP430_MCU_CLASS(oc);

    mc->clock_type = CLOCK_BCM;
...
static void msp430f1611_class_init(ObjectClass *oc, void *data)
{
    MSP430Class *mc = MSP430_MCU_CLASS(oc);
    int i;

    mc->flash_size = 48 * KiB;
    mc->sram_size = 10 * KiB;
    mc->bsl_size = KiB;
    ...
    mc->timer[0].timers = 3;
    mc->timer[0].aclk_cci = 2;
    mc->timer[0].caout_cci = 1;
    mc->timer[1].type = TYPE_MSP430_TIMER_B;
    mc->timer[1].timers = 7;
    mc->timer[1].aclk_cci = 6;
    mc->usart[0].present = true;
    mc->usart[0].i2c = true;
    mc->usart[1].present = true;

    msp430_class_render(mc, "MSP430F1611");
}
...
static void msp430f4794_class_init(ObjectClass *oc, void *data)
{
    MSP430Class *mc = MSP430_MCU_CLASS(oc);
    int i;

    mc->flash_size = 60 * KiB - 256;
    mc->sram_size = 2 * KiB + 512;
    mc->bsl_size = KiB;
    mc->fllp_has_xts = true;
    mc->fllp_has_sel = true;
    mc->mpy_type = TYPE_MSP430_MPY32;
    mc->ports_have_ren = true;
    for (i = 0; i < 5; i++) {
        mc->port[i].present = true;
    }
    mc->port16[0].present = true;
    mc->port16[1].present = true;
    mc->timer[0].type = TYPE_MSP430_TIMER_A;
    mc->timer[0].timers = 3;
    mc->timer[0].aclk_cci = 2;
    mc->timer[0].caout_cci = 1;
    ...
    mc->timer[1].timers = 3;
    mc->usci_a[0].present = true;
    mc->usci_a[1].present = true;
    mc->usci_b[0].present = true;
    mc->usci_b[1].present = true;

    msp430_class_render(mc, "MSP430F4791");
}
...
static const TypeInfo msp430_mcu_types[] = {
    ...
    {
        .name           = TYPE_MSP430X1XX_MCU,
        .parent         = TYPE_MSP430_MCU,
        .class_init     = msp430x1xx_class_init,
        .abstract       = true,
    },
    {
        .name           = TYPE_MSP430F1611_MCU,
        .parent         = TYPE_MSP430X1XX_MCU,
        .class_init     = msp430f1611_class_init,
    },
    {
    ...
    {
        .name           = TYPE_MSP430F4794_MCU,
        .parent         = TYPE_MSP430X4XX_MCU,
        .class_init     = msp430f4794_class_init,
    },
};
