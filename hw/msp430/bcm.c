                      ...
                      uint8_t *best_bcsctl1)
{
    ...
    for (rsel = 0; rsel < (bc->plus ? 16 : 8); rsel++) {
        for (dco = 0; dco < 8; dco++) {
            for (mod = 0; mod < 32; mod++) {
                ...
                if (error < best_error) {
                    if (freq == 1000000 && error < 1000) {
                        printf("HERE %u %u %u -> %" PRIu64 " (%" PRIu64 " < %" PRIu64 ")\n",
                               rsel, dco, mod, dcoclk, error, best_error);
                    }
    ...
    printf("GOT %u %u %u -> %" PRIu64 " (%" PRIu64 ")\n",
           rsel, dco, mod, bcm_dcoclk(bcm, rsel, dco, mod), best_error);
}
