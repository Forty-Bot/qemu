...
/* Based on SLAS368G, adjusted for DCO=0 assuming S_DCO=1.12 */
static const uint64_t bcm_dco_freq[] = {
    92531,
    128120,
    199298,
    334537,
...
static const uint64_t bcmp_dco_freq[] = {
    95260,
    119075,
    166705,
    238150,
...
static void bcm_set_clocks(MSP430BCMState *bcm)
{
    ...
    if (bc->plus) {
        dcoclk = bcmp_dco_freq[FIELD_EX8(bcm->bcsctl1, BCSCTL1, RSEL)];
        for (i = 0; i < dco; i++) {
            dcoclk += dcoclk * 2 / 25;
        }
    } else {
        dcoclk = bcm_dco_freq[FIELD_EX8(bcm->bcsctl1, BCSCTL1, RSEL_BCM)];
        for (i = 0; i < dco; i++) {
            dcoclk += dcoclk * 3 / 25;
        }
    }
...
static void bcm_reset_hold(Object *obj)
{
