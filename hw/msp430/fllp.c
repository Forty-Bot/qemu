...
static void fllp_set_oscoff(void *opaque, int irq, int level)
{
    MSP430FLLPState *fllp = opaque;
...
static void fllp_reset(MSP430FLLPState *fllp, bool por)
{
...
static void fllp_reset_hold(Object *obj, ResetType type)
{
    MSP430FLLPState *fllp = MSP430_FLLP(obj);

    fllp_reset(fllp, true);
}

static void fllp_puc(void *opaque, int irq, int level)
{
    if (level) {
        fllp_reset(opaque, false);
    }
}
...
static void fllp_init(Object *obj)
{
    ...
    qdev_init_gpio_in_named(DEVICE(d), fllp_puc, "puc", 1);
    qdev_init_gpio_in_named(DEVICE(d), fllp_set_cpuoff, "cpuoff", 1);
    qdev_init_gpio_in_named(DEVICE(d), fllp_set_oscoff, "oscoff", 1);

    fllp->vlo_freq = g_random_int_range(4000, 20000);
}
...
static const VMStateDescription vmstate_fllp = {
    ...
    .fields = (VMStateField[]) {
        ...
        VMSTATE_UINT64(vlo_freq, MSP430FLLPState),
        VMSTATE_END_OF_LIST()
    }
};
