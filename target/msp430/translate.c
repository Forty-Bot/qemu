...
static void gen_c_arith(TCGv res, bool byte)
{
...
static void setup_dst(DisasContext *ctx, int rd, int ad, bool byte)
{
    ...
    off = cpu_ldsw_code(ctx->env, ctx->base.pc_next);
    ctx->base.pc_next += 2;
    if (rd == R_SR) {
        if (!byte) {
            off &= 0xfffe;
...
static bool put_dst(DisasContext *ctx, TCGv val, int rd, int ad, bool byte)
{
    ...
    if (rd == R_SR) {
        ...
        tcg_gen_setcond_i32(TCG_COND_NE, cpu_pending_gie, cpu_gie, cpu_pending_gie);
...
static bool trans_CALL(DisasContext *ctx, arg_CALL *a)
{
    ...
    tcg_gen_qemu_st_i32(cpu_regs[R_PC], cpu_regs[R_SP], 0, MO_TEUW);
...
{
    if (carry) {
        tcg_gen_sub_i32(res, dst, cpu_c);
        tcg_gen_sub_i32(res, res, src);
    } else {
...
static void msp430_tr_translate_insn(DisasContextBase *dcbase, CPUState *cs)
{
    ...
    ctx->base.pc_next += 2;
    if (!decode(ctx, insn)) {
        //qemu_printf("%04x: unsupported instruction %04x\n", ctx->base.pc_next, insn);
