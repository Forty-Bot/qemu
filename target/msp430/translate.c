...
{
    if (carry) {
        tcg_gen_sub_i32(res, src, cpu_c);
        tcg_gen_sub_i32(res, res, dst);
    } else {
        tcg_gen_sub_i32(res, src, dst);
    }
    gen_c_arith(res, byte);
    tcg_gen_andi_i32(res, res, byte ? 0xff : 0xffff);
...
{
    DisasContext *ctx = container_of(dcbase, DisasContext, base);
    uint16_t insn = cpu_ldsw_code(ctx->env, ctx->base.pc_next);
